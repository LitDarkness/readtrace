...
这份教程是 CLI 的完整复现流程，从空 Workspace 开始，完成三种输入：一个 PDF、一个 Markdown 文件和两张图片。示例文件名只是占位符，本仓库不携带书籍、外部 PDF 或个人图片。第一次使用或希望直接操作 GUI 时，请先看 [`QUICK_START.md`](QUICK_START.md)；Windows 的 Tesseract/Poppler 安装也在那里按步骤说明。正文保留已在 Windows 验证的 PowerShell 记录。macOS 先按 [`RELEASE_GUIDE.md`](RELEASE_GUIDE.md) 或 [`QUICK_START.md`](QUICK_START.md) 安装依赖，再将 Windows 路径换成正斜杠路径、PowerShell 反引号换成 shell 反斜杠；所有 ReadTrace CLI 参数保持不变。命令都从项目根目录运行，也就是：
...
项目根目录的 `.env` 已配置 Tesseract 和 Poppler 时，先检查 OCR：

```powershell
cargo run --quiet -p readtrace-cli -- ocr-check
```

需要真实 LLM 时，再确认 `.env` 至少包含 `READTRACE_BASE_URL`、`READTRACE_ENDPOINT_PATH`、`READTRACE_MODEL` 和 `READTRACE_API_KEY_ENV`；没有 Provider 时可以把后面的 repair/answer 命令改成 `mock`。不要把 Key 写进命令或提交到 Git。
...
```text
tmp/tutorial-input/sample-book.pdf
tmp/tutorial-input/sample.md
...
$pdf = cargo run --quiet -p readtrace-cli -- --format json `
  import-file $vault ".\tmp\tutorial-input\sample-book.pdf" --mode book | ConvertFrom-Json
...
$md = cargo run --quiet -p readtrace-cli -- --format json `
  import-file $vault ".\tmp\tutorial-input\sample.md" --mode game_dialogue | ConvertFrom-Json
$mdBatch = $md.batch_id
```
...
它仍然包含 `rt:block` 来源锚点，因此检索和引用可以回到原始 `sample.md`。`direct-clean` 只接受一个 TXT/MD batch；多文件请先用 `sources` 查看单元，再走合并计划。
...
PDF 会显示两段进度：先是栅格化页数，再是 Tesseract 页数。例如 25 页文件会出现 `rendering PDF (25 pages)` 和 `OCR page n/25`。PDF 的一个文件也可能包含很多页，但仍然只算一个 source；页级锚点会写成 `sample-book.pdf:page:1`、`sample-book.pdf:page:2` 等。
...
GUI 可以用同一组自有素材复现这条链路。为了让仓库与外部交付材料、个人书籍和测试图片解耦，这里只保留文件类型、操作顺序和预期结果，不固定本机的 batch ID、文件名或费用数字。

| 输入 | GUI 操作 | 预期结果 |
| --- | --- | --- |
| 一份多页 PDF | 导入队列选择“只导入”，再在处理页执行 OCR → 规范化 → repair | 页面显示栅格化和逐页 OCR 进度；repair 完成后才允许生成 clean |
| 一份 `.md` 或 `.txt` | 选择“直接发布到 clean”，或选择批次处理让模型改写 | 直接发布不调用 LLM；批次处理会生成可回看的 repair checkpoint |
| 两张 PNG/JPG | 导入同一批次，执行真实 OCR，再选择 Mock、HTTP 或 Codex repair | 每张图保留独立 source/page 锚点；可人工编辑合并计划，也可确认自动顺序 |

完成处理后，在“文件浏览”打开 `clean/` 中的 Markdown，检查并保存人工修改；在“检索”页确认只能搜到 clean；在“阅读与问答”页用“添加引用”多选 clean 文件，再提问“这段剧情讲了什么？”。回答应显示引用文件和来源锚点，后台应显示任务终态及 Token/费用。

本仓库不携带上述输入素材。需要记录一次真实演示时，建议在仓库外创建独立的 `demo-input/` 和 `demo-workspace/`，完成后只把脱敏的截图或文字结果放入发布说明，不把书籍、外部 PDF、API Key 或运行台账提交到 Git。

## 9. 历史验证说明

项目开发期间已经用 PDF、Markdown、TXT 和多张图片验证过真实 OCR、Mock/HTTP/Codex Provider、clean 发布、搜索、合并和引用问答。历史 batch、输入文件和调用台账只存在于开发机的 Workspace/`deliverables/` 中，不能作为新设备的固定依赖；按本教程重新生成的 batch ID 和费用会不同。
