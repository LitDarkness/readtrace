...
CLI `usage` 和 Web `/api/usage` 提供批次/全 Vault 汇总，并显示 cached input Token。扫描模式会读取根目录下所有 `.jsonl` 并按 `call_id` 合并，所以 `--ledger tmp\...jsonl` 的测试调用也会进入汇总；Codex CLI 的 usage 不可见时保留 unknown。开发期间的外部订阅费用和人时不从这份运行时台账推断，需另行人工记录。
